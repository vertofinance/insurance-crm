-- Drop and Recreate Insurance CRM Database
-- This script will completely recreate the database with enhanced features

-- First, disconnect all connections to the database
SELECT pg_terminate_backend(pid) 
FROM pg_stat_activity 
WHERE datname = 'insurance_crm' AND pid <> pg_backend_pid();

-- Drop the existing database
DROP DATABASE IF EXISTS insurance_crm;

-- Create the new database
CREATE DATABASE insurance_crm;

-- Connect to the new database and run the enhanced schema
\c insurance_crm
